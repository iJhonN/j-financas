...
export default function LancamentoPage() {
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // LÓGICA DE VALOR:
      // Se RECORRENTE: Repete o valor CHEIO 12x.
      // Se CRÉDITO (não recorrente): Divide o valor total pelas parcelas.
      let numRepeticoes = 1;
      let valorPorLancamento = valorComSinal;

      if (recorrente) {
        numRepeticoes = 12;
        valorPorLancamento = valorComSinal;
      } else if (tipoPagamento === 'Crédito') {
        numRepeticoes = parcelas;
        valorPorLancamento = parseFloat((valorComSinal / numRepeticoes).toFixed(2));
      }
      ...
      for (let i = 0; i < numRepeticoes; i++) {
        ...
        // Lógica de Vencimento de Cartão (Apenas para Crédito Parcelado)
