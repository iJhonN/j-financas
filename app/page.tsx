    ...
    <div className="min-h-screen bg-[#0a0f1d] p-2 md:p-8 text-white font-black antialiased overflow-x-hidden pb-24 italic">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6 font-black leading-none">
        ...
        <div className="relative font-black leading-none">
          <button onClick={() => setIsFilterMenuOpen(!isFilterMenuOpen)} className="w-full bg-[#111827] p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl border-b-8 border-amber-500 flex flex-col justify-between h-32 md:h-36 text-left leading-none font-black italic">
            <span className="text-white/40 text-[7px] md:text-[10px] uppercase tracking-widest font-black">Filtrar por:</span>
          ...
          {isFilterMenuOpen && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-[#111827] border-2 border-slate-800 rounded-3xl shadow-2xl z-[400] overflow-hidden font-black">
              <div className="p-2 max-h-64 overflow-y-auto uppercase text-[10px] italic">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6 leading-none">
        <div className="lg:col-span-2 space-y-6 leading-none">
          ...
          <div className="bg-[#111827] p-5 md:p-8 rounded-[2rem] border border-slate-800 shadow-2xl font-black leading-none">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {cartoes.map(c => (
                <div key={c.id} className="p-4 border-2 border-slate-800 rounded-2xl flex justify-between items-center bg-slate-950/50 hover:border-blue-500 transition-all leading-none italic">
        ...
        <div className="bg-[#111827] p-5 md:p-8 rounded-[2rem] border border-slate-800 h-full overflow-hidden flex flex-col shadow-2xl min-h-[500px] leading-none">
          <h2 className="text-white font-black mb-4 uppercase text-[10px] tracking-widest px-1 italic">Últimos Lançamentos</h2>
          <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar flex-1 font-black leading-none">
            {transacoesFiltradas.map((t) => (
              <div key={t.id} className="flex justify-between items-center p-4 bg-slate-800/40 rounded-2xl border border-slate-800 hover:border-slate-600 leading-none italic">
      ...
      {/* MODAL NOVO LANÇAMENTO - CORRIGIDO PARA MOBILE */}
      {isModalOpen && !isExpired && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[4000] animate-in fade-in zoom-in-95 leading-none font-black italic">
          <form onSubmit={handleSalvarGasto} className="bg-[#111827] w-full max-w-md rounded-[3rem] p-6 md:p-8 border-4 border-slate-800 shadow-2xl text-white font-black">
            <div className="flex justify-between items-center mb-6 leading-none italic">
            ...
            <div className="space-y-4 font-black leading-none italic">
              <div className="flex gap-2 p-1 bg-slate-800 rounded-2xl leading-none font-black">
              ...
              <div className="relative leading-none font-black">
              ...
              {/* LAYOUT VERTICAL PARA DATA E MÉTODO */}
              ...
              <div className="p-4 bg-slate-950/50 rounded-2xl border border-slate-800 space-y-3 font-black leading-none italic">
                <div className="flex items-center justify-between leading-none font-black">
                  <span className="text-[10px] uppercase text-white flex items-center gap-2"><RefreshCcw size={14} className="text-blue-400"/> Recorrente?</span>
              ...
              <button type="submit" className={`w-full ${theme.primary} text-white py-5 rounded-[2rem] shadow-xl uppercase text-[10px] mt-2 active:scale-95 transition-all font-black italic`}>Finalizar Lançamento</button>
            </div>
          </form>
        </div>
      )}
      ...
      {isCardModalOpen && !isExpired && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[5000] animate-in fade-in zoom-in-95 leading-none font-black italic">
          <form onSubmit={handleSalvarCartao} className="bg-[#111827] w-full max-w-sm rounded-[3rem] p-8 md:p-10 border-4 border-slate-800 shadow-2xl text-white italic leading-none">
            <h2 className="text-xl mb-10 text-center uppercase tracking-widest font-black">Novo Cartão</h2>
      ...
      {isSaldoModalOpen && !isExpired && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[5000] animate-in fade-in zoom-in-95 leading-none font-black italic">
          <div className="bg-[#111827] w-full max-w-sm rounded-[3rem] p-10 border-4 border-slate-800 shadow-2xl text-white italic leading-none">
            ...
            }} className="w-full bg-emerald-600 py-5 rounded-[2rem] uppercase text-[10px] shadow-lg active:scale-95 font-black italic">Confirmar</button>
      ...
      {isConfigModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[5000] animate-in fade-in zoom-in-95 leading-none font-black italic">
          ...
          }} className="bg-[#111827] w-full max-w-sm rounded-[3rem] p-10 border-4 border-slate-800 shadow-2xl text-white italic font-black leading-none">
            <div className="flex justify-between items-center mb-8 px-1"><h2 className="text-xl uppercase tracking-widest font-black">Ajustes</h2><button type="button" onClick={() => setIsConfigModalOpen(false)} className="bg-slate-800 p-2 rounded-full text-slate-500 leading-none"><X size={20} /></button></div>
  ...
  return (
    <div className={`${color} p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl transition-transform active:scale-[0.98] border-black/20 flex flex-col justify-between h-32 md:h-36 text-white text-left font-black italic leading-none`}>
      <div className="flex justify-between items-start w-full leading-none font-black italic">
        <span className="text-white/20 font-black text-[7px] md:text-[10px] uppercase tracking-widest leading-none">
          {title}
        </span>
        <div className="p-1.5 md:p-3 bg-white/5 rounded-xl backdrop-blur-md border border-white/5 opacity-50 leading-none">
          {icon}
        </div>
      </div>
      <div className="text-sm md:text-2xl font-black truncate uppercase px-1 leading-tight">
        {value}
      </div>
    </div>
  );
