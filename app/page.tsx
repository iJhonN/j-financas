...
export default function App() {
  ...
  return (
    <>
      {alertConfig.show && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[2000] animate-in fade-in slide-in-from-top-4 duration-300 px-4 w-full max-w-sm">
...
function AuthScreen({ theme, setView, setUser, showAlert }: any) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (authMode === 'login') {
      ...
      if (error) return showAlert("E-mail ou senha inválidos", 'error');
      setUser(data.user);
      setView('dashboard');
    } else {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-955 text-white font-black italic">
      <form onSubmit={handleAuth} className="bg-slate-900 w-full max-w-md rounded-[2.5rem] p-8 md:p-10 border-4 border-slate-800 shadow-2xl">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-xl md:text-2xl font-black uppercase tracking-tighter px-1">J FINANÇAS</h1>
        </div>
        <div className="space-y-4">
          ...
          <button type="submit" className={`w-full ${theme.primary} text-white font-black py-5 rounded-[2rem] shadow-xl ${theme.hover} transition-all uppercase text-sm mt-4 tracking-widest italic`}>
...
function Dashboard({ user, theme, currentThemeName, setTheme, onLogout, showAlert }: any) {
  ...
  // TRAVA DE SEGURANÇA RE-IMPLEMENTADA
  ...
  const handleSalvarGasto = async (e: React.FormEvent) => {
    ...
    if (!error) { fetchDados(); setIsModalOpen(false); setDescricao(''); setValorDisplay(''); showAlert("Gasto salvo!", 'success'); }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-955 p-2 md:p-8 text-white font-sans overflow-x-hidden pb-20 italic font-black">
        ...
        <div className="flex justify-between items-center w-full">
          ...
          <div className="flex gap-2">
            {isAdmin && (
              ...
              }} className="bg-amber-500 text-slate-950 p-2.5 rounded-full active:scale-95 transition-all shadow-lg shadow-amber-500/20">
                <ShieldCheck size={20} />
              </button>
            )}
            <div className="relative">
              ...
              {isProfileMenuOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-slate-900 border-2 border-slate-800 rounded-[2rem] shadow-2xl z-[110] overflow-hidden animate-in fade-in slide-in-from-top-2 font-black">
                  <div className="p-2 space-y-1">
                    <button onClick={() => { setIsProfileMenuOpen(false); setIsConfigModalOpen(true); }} className="w-full flex items-center gap-3 p-4 hover:bg-slate-800 rounded-2xl transition-all">
                      <Settings className={theme.text} size={18} />
                      <span className="text-[10px] uppercase">Configurações do App</span>
                    </button>
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 shadow-2xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {cartoes.map(c => (
                      <div key={c.id} className="p-4 md:p-5 border-2 border-slate-800 rounded-2xl flex justify-between items-center bg-slate-950/50 hover:border-blue-500 transition-all group font-black italic">
                        <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={async () => { if(confirm("Remover cartão?")) { await supabase.from('cartoes').delete().eq('id', c.id); fetchDados(); showAlert("Cartão removido", 'success'); } }} className="text-slate-600 hover:text-rose-500 transition-all"><Trash2 size={16} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {transacoesFiltradas.length > 0 ? transacoesFiltradas.map((t) => (
                <div key={t.id} className="flex justify-between items-center p-4 bg-slate-800/40 rounded-2xl border border-slate-800 hover:bg-slate-800/60 transition-all leading-normal">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-black text-xs text-rose-500 italic px-1 whitespace-nowrap leading-tight">R$ {formatarMoeda(t.valor)}</span>
                    <button onClick={async () => { if(confirm("Apagar gasto?")) { await supabase.from('transacoes').delete().eq('id', t.id); fetchDados(); showAlert("Gasto removido", 'success'); } }} className="text-slate-700 hover:text-rose-500 transition-all"><Trash2 size={14} /></button>
                  </div>
                </div>
              )) : <p className="text-center text-slate-600 font-black text-[9px] uppercase mt-20">Nenhum registro</p>}
            </div>
          </div>
      </div>

      {/* MODAL CONFIGURAÇÕES (RESTAURADO) */}
      {isConfigModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1000] animate-in fade-in zoom-in-95 duration-300 font-black italic">
          <form onSubmit={handleUpdateProfile} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4">
              ...
              <input value={novoNome} onChange={(e) => setNovoNome(e.target.value)} placeholder="NOME" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-white uppercase text-[10px] font-black" />
      ...
      {/* MODAL GASTO (CENTRALIZADO E AJUSTADO) */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1000] animate-in fade-in zoom-in-95 duration-300 font-black italic">
          <form onSubmit={handleSalvarGasto} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                <span className={`absolute left-4 top-1/2 -translate-y-1/2 ${theme.text} text-sm px-1`}>R$</span>
              ...
              <select value={formaPagamento} onChange={(e) => setFormaPagamento(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white uppercase text-[10px] outline-none">
                ...
                {cartoes.map(c => (<option key={c.id} value={c.banco}>{c.banco} ({c.nome_cartao})</option>))}
              </select>
              ...
              <button type="submit" className={`w-full ${theme.primary} text-white py-5 rounded-[2rem] shadow-xl ${theme.hover} active:scale-95 transition-all uppercase text-xs mt-2 italic font-black leading-normal`}>Lançar Agora</button>
            </div>
          </form>
        </div>
      )}
      ...
      {/* MODAL CARTÃO (COM TRAVA DE SEGURANÇA) */}
      {isCardModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1000] animate-in fade-in duration-300 font-black italic">
          <form onSubmit={handleSalvarCartao} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4 font-black">
              ...
              <div>
                ...
                <p className="text-[7px] text-slate-500 ml-2 mt-1 uppercase">Apenas letras. Máximo 12 caracteres.</p>
              </div>
