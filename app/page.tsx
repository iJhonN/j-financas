  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] p-2 md:p-8 text-white font-black antialiased overflow-x-hidden pb-10 italic uppercase relative leading-none">
      ...
      {tourStep > 0 && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6 pointer-events-none">
          <div className="pointer-events-auto bg-gradient-to-b from-blue-600 to-blue-800 p-8 rounded-[3rem] border-[3px] border-white/20 shadow-2xl max-w-sm w-full animate-in zoom-in relative text-center">
            <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-white text-blue-700 px-4 py-1.5 rounded-full text-[10px] shadow-xl border-2 border-blue-600">PASSO {tourStep} / 5</div>
            <button onClick={() => {setTourStep(0); localStorage.setItem('wolf_tour_complete', 'true');}} className="absolute top-4 right-4 bg-white/10 p-2 rounded-full"><X size={18}/></button>
            <div className="py-4">
              <div className="mb-4">{TOUR_CONTENT[tourStep-1].icon}</div>
              <h3 className="text-2xl mb-4 italic">{TOUR_CONTENT[tourStep-1].title}</h3>
              <p className="text-[10px] normal-case mb-8 italic text-blue-50 leading-tight">{TOUR_CONTENT[tourStep-1].desc}</p>
              <button onClick={() => tourStep === 5 ? setTourStep(0) : setTourStep(s => s + 1)} className={`w-full py-5 rounded-2xl text-[12px] font-black uppercase ${tourStep === 5 ? 'bg-emerald-500 text-white' : 'bg-white text-blue-700'}`}>
      ...
      <header className="mb-6 bg-[#111827] p-4 md:p-6 rounded-[2rem] border border-slate-800 shadow-2xl">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => router.push('/tutorial')} className="bg-slate-800 text-blue-400 p-2.5 rounded-full border border-blue-500/30 shadow-lg"><HelpCircle size={20}/></button>
            <button onClick={() => router.push('/perfil')} className="bg-slate-800 text-slate-300 p-2.5 rounded-full border border-slate-700 shadow-lg"><UserCircle size={20}/></button>
          </div>
        </div>
        <div className="flex gap-2">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6">
        ...
        <div className="bg-[#111827] p-4 rounded-[1.5rem] border-b-8 border-amber-500 h-32 flex flex-col justify-between shadow-2xl">
           <div className="flex justify-between items-center text-[9px] border-b border-white/10 pb-2">
           ...
           <button onClick={() => setIsFilterMenuOpen(!isFilterMenuOpen)} className="w-full flex items-center justify-between text-[9px] bg-slate-800/50 p-2 rounded-lg border border-slate-700 mt-2 truncate">
             <span className="truncate">{filtroCartao}</span><ChevronDown size={12}/>
           </button>
           {isFilterMenuOpen && (
             <div className="absolute bottom-1/4 left-4 right-4 bg-[#111827] border-2 border-slate-800 rounded-xl shadow-2xl z-[500] max-h-40 overflow-y-auto custom-scrollbar">
               <button onClick={() => { setFiltroCartao('Todos'); setIsFilterMenuOpen(false); }} className="w-full text-left p-3 border-b border-slate-800 text-[9px] hover:bg-slate-800">Todos</button>
               {cartoes.map(c => <button key={c.id} onClick={() => { setFiltroCartao(`${c.banco} - ${c.nome_cartao}`); setIsFilterMenuOpen(false); }} className="w-full text-left p-3 border-b border-slate-800 text-[9px] hover:bg-slate-800">{c.banco} - {c.nome_cartao}</button>)}
             </div>
           )}
        </div>
      </div>
      ...
      {/* CHART */}
      <div className="bg-[#111827] p-6 rounded-[2.5rem] border border-slate-800 shadow-2xl h-80 mb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-[#111827] p-5 md:p-8 rounded-[2rem] border border-slate-800 shadow-2xl">
          <h2 className="mb-6 text-[10px] tracking-widest">Meus Cartões</h2>
            ...
            {cartoes.map(c => (
              <div key={c.id} className="p-4 border-2 border-slate-800 rounded-2xl flex justify-between bg-slate-950/50 hover:border-blue-500 transition-all">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-[8px] text-slate-500 uppercase">{c.banco}</p><p className="text-xs uppercase italic">{c.nome_cartao}</p></div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => router.push('/cartoes')} className="text-slate-600 hover:text-white"><Pencil size={18}/></button>
                  <button onClick={async () => { if(confirm("Apagar?")) { await supabase.from('cartoes').delete().eq('id', c.id); fetchDados(user.id); showAlert("Cartão removido!"); } }} className="text-slate-600 hover:text-rose-500"><Trash2 size={18}/></button>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ACTIVITY LIST */}
        <div className="bg-[#111827] p-5 md:p-8 rounded-[2rem] border border-slate-800 shadow-2xl min-h-[500px] flex flex-col">
          <h2 className="mb-4 text-[10px] tracking-widest">Atividade</h2>
          <div className="relative mb-4">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={14} />
            <input type="text" placeholder="BUSCAR..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-slate-800/50 border border-slate-700 rounded-xl py-2 pl-9 pr-4 text-[9px] outline-none" />
          </div>
          <div className="space-y-3 overflow-y-auto flex-1 custom-scrollbar">
            {transacoesFiltradas.map((t) => (
              <div key={t.id} className={`flex justify-between items-center p-4 rounded-2xl border ${t.pago ? 'bg-slate-800/40 border-slate-800' : 'bg-rose-900/10 border-rose-900/30'} ${tourStep === 5 ? 'relative z-[10001] ring-4 ring-white shadow-[0_0_0_9999px_rgba(10,15,29,0.7)]' : ''}`}>
                <div className="flex items-center gap-3">
                  <button onClick={async () => { await supabase.from('transacoes').update({ pago: !t.pago }).eq('id', t.id); fetchDados(user.id); }} className={`p-1.5 rounded-full ${t.pago ? 'text-emerald-500 bg-emerald-500/10' : 'text-slate-500 bg-slate-800'}`}>{t.pago ? <CheckCircle size={18}/> : <Circle size={18}/>}</button>
                  <div className="leading-tight truncate max-w-[120px]"><p className="text-[10px] uppercase">{t.descricao}</p><p className={`text-[7px] uppercase ${theme.text}`}>{t.forma_pagamento}</p></div>
                </div>
      ...
      {isSaldoModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-[6000]">
          <div className="bg-[#111827] w-full max-w-sm rounded-[3rem] p-10 border-4 border-slate-800 shadow-2xl relative text-center italic">
            <button onClick={() => setIsSaldoModalOpen(false)} className="absolute top-6 right-6 p-2 bg-slate-800 rounded-full text-slate-400"><X size={20}/></button>
            ...
            <div className="relative mb-6">
              ...
              <input type="text" value={saldoDisplay} onChange={(e) => {
                ...
                setSaldoDisplay(v);
              }} placeholder="R$ 0,00" className="w-full p-5 bg-slate-800 rounded-2xl border-2 border-slate-700 text-emerald-500 text-xl font-black text-center outline-none" />
            </div>
            <button onClick={async () => { 
              ...
              }} className="w-full bg-emerald-600 py-5 rounded-[2rem] text-[10px] uppercase font-black">Confirmar Novo Saldo</button>
          </div>
        </div>
      )}
...
function ActionButton({ icon, label, active, onClick, color, themeColor }: any) {
  const base = "flex-1 p-3 rounded-2xl border text-[10px] flex items-center justify-center gap-2 transition-all font-black active:scale-95";
  const colors: any = {
    emerald: "border-emerald-800/50 bg-emerald-900/20 text-emerald-400",
    slate: "border-slate-700 bg-slate-800/50 text-slate-300",
    theme: `${themeColor} text-white`
  };
...
function Card({ title, value, icon, border, onClick }: any) {
  return (
    <div onClick={onClick} className={`bg-[#111827] p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] border-b-8 ${border} h-32 md:h-36 flex flex-col justify-between shadow-2xl transition-all ${onClick ? 'cursor-pointer hover:scale-105 active:scale-95' : ''}`}>
      <div className="flex justify-between items-start opacity-50"><span className="text-[7px] md:text-[10px] uppercase">{title}</span>{icon}</div>
      <div className="text-sm md:text-2xl truncate">{value}</div>
    </div>
  );
}