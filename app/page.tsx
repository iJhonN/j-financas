...
import { 
  ...
  UserCircle, ShieldCheck, Loader2, Filter, ChevronDown, Bell, BellOff, Settings, AlertCircle, CheckCircle, Users, Eye, EyeOff 
} from 'lucide-react';
    ...
    <>
      ...
      {view === 'auth' ? (
        <div className="min-h-screen flex items-center justify-center p-4 bg-slate-955 text-white">
          <form onSubmit={handleAuth} className="bg-slate-900 w-full max-w-md rounded-[2.5rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl font-black italic">
...
function Dashboard({ user, onLogout, showAlert }: any) {
  ...
  const toggleNotifications = async () => {
    ...
    if (permission === 'granted') showAlert("Notificações ON", 'success');
    ...
    <div className="min-h-screen bg-slate-955 p-2 md:p-8 text-white font-sans overflow-x-hidden pb-20">
      <header className="flex flex-col gap-4 mb-6 bg-slate-900 p-4 md:p-6 rounded-[2rem] border border-slate-800 shadow-2xl relative">
        <div className="flex justify-between items-center w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg md:text-xl font-black uppercase tracking-tighter italic px-1 leading-normal">J FINANÇAS</h1>
              ...
              {isProfileMenuOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-slate-900 border-2 border-slate-800 rounded-[2rem] shadow-2xl z-[110] overflow-hidden animate-in fade-in slide-in-from-top-2">
                  <div className="p-3 space-y-1">
                    ...
                    <button onClick={onLogout} className="w-full flex items-center gap-3 p-4 hover:bg-rose-900/20 text-rose-500 rounded-2xl transition-all border-t border-slate-800/50 mt-1 font-black italic uppercase text-[10px]">
                      <LogOut size={18} /> Sair
                    </button>
                  </div>
                </div>
        ...
        <div className="flex gap-2 font-black italic">
          ...
          <button onClick={() => setIsModalOpen(true)} className="w-full md:w-auto bg-blue-600 text-white p-3.5 rounded-2xl shadow-lg text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-blue-700 active:scale-95"><Plus size={18} /> Novo Gasto</button>
        </div>
      </header>
        ...
        <div className="relative">
          <button onClick={() => setIsFilterMenuOpen(!isFilterMenuOpen)} className="w-full bg-slate-900 p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl border-b-8 border-amber-500 flex flex-col justify-between h-32 md:h-36 text-left active:scale-95 transition-all">
            <div className="flex justify-between items-start w-full">
              <span className="text-white/40 font-black text-[7px] md:text-[10px] uppercase tracking-widest leading-tight italic">Filtrar:</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            {/* RESTAURAÇÃO: LISTA DE CARTÕES COM LOGOS  */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {cartoes.map(c => (
                    <div key={c.id} className="p-4 md:p-5 border-2 border-slate-800 rounded-2xl flex justify-between items-center bg-slate-950/50 hover:border-blue-500 transition-all group font-black italic">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-slate-800 rounded-xl flex items-center justify-center border border-slate-700 p-1.5 overflow-hidden flex-shrink-0">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => { setEditingCardId(c.id); setBanco(c.banco); setNomeCartao(c.nome_cartao); setVencimento(c.vencimento.toString()); setIsCardModalOpen(true); }} className="text-slate-600 hover:text-blue-500 transition-all"><Pencil size={16} /></button>
        ...
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[200] animate-in fade-in zoom-in-95 duration-300 font-black italic">
          <form onSubmit={handleSalvarGasto} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4">
              ...
              <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-[2rem] shadow-xl hover:bg-blue-700 active:scale-95 transition-all uppercase text-xs mt-2 italic font-black leading-normal">Lançar Agora</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[200] animate-in fade-in zoom-in-95 duration-300 font-black italic">
          <form onSubmit={handleUpdateProfile} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4">
              ...
              <button type="submit" className="w-full bg-blue-600 py-5 rounded-[2rem] uppercase text-xs mt-2 font-black italic">Salvar Dados</button>
      ...
      {isCardModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[200] animate-in fade-in zoom-in-95 duration-300 font-black italic">
          <form onSubmit={handleSalvarCartao} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4">
              <input value={banco} onChange={(e) => setBanco(e.target.value)} placeholder="BANCO (EX: NUBANK)" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-white uppercase text-[10px] font-black" required />
              ...
              <button type="submit" className="w-full bg-blue-600 py-5 rounded-[2rem] uppercase text-xs mt-2 active:scale-95 italic font-black leading-normal">Salvar Cartão</button>
