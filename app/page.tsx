      ...
      {alertConfig.show && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[1000] animate-in fade-in slide-in-from-top-4 duration-300 px-4 w-full max-w-sm">
          <div className={`flex items-center gap-3 p-4 rounded-2xl border-2 shadow-2xl backdrop-blur-xl ${alertConfig.type === 'error' ? 'bg-rose-950/80 border-rose-500 text-rose-200' : 'bg-emerald-950/80 border-emerald-500 text-emerald-200'}`}>
            ...
            <p className="text-xs font-black uppercase tracking-widest leading-normal px-1">{alertConfig.msg}</p>
        ...
        <div className="min-h-screen flex items-center justify-center p-4 bg-slate-955 text-white">
          <form onSubmit={handleAuth} className="bg-slate-900 w-full max-w-md rounded-[2.5rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl">
            ...
            <div className="space-y-4 font-bold">
              {authMode === 'signup' && (
                <input type="text" placeholder="Nome completo" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-white focus:border-blue-600 uppercase text-xs" required />
              )}
              <input type="email" placeholder="E-mail" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-white focus:border-blue-600 uppercase text-xs" required />
              ...
              <button type="submit" className="w-full bg-blue-600 text-white font-black py-5 rounded-[2rem] shadow-xl hover:bg-blue-700 transition-all uppercase text-sm mt-4 tracking-widest leading-normal">
    ...
    <div className="min-h-screen bg-slate-955 p-2 md:p-8 text-white font-sans overflow-x-hidden pb-20">
      <header className="flex flex-col gap-4 mb-6 bg-slate-900 p-4 md:p-6 rounded-[2rem] border border-slate-800 shadow-2xl relative">
        <div className="flex justify-between items-center w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg md:text-xl font-black uppercase tracking-tighter leading-normal italic px-1">J FINANÇAS</h1>
          ...
          <div className="flex gap-2">
            {isAdmin && (
              <button onClick={() => setIsAdminMenuOpen(true)} className="bg-amber-500 text-slate-950 p-2.5 rounded-full shadow-lg border-2 border-amber-400/50 hover:scale-110 transition-all">
                <ShieldCheck size={20} />
              </button>
            )}
            <div className="relative">
              ...
              {isProfileMenuOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-slate-900 border-2 border-slate-800 rounded-[2rem] shadow-2xl z-[110] overflow-hidden">
                  <div className="p-3 space-y-1">
                    <button onClick={() => { supabase.auth.signOut(); window.location.reload(); }} className="w-full flex items-center gap-3 p-4 hover:bg-rose-900/20 text-rose-500 rounded-2xl transition-all font-black uppercase text-[10px]">
                      <LogOut size={18} /> Sair
                    </button>
                  </div>
                </div>
        ...
        <div className="flex gap-2 font-black">
          <button onClick={() => setIsSaldoModalOpen(true)} className="flex-1 bg-emerald-900/20 text-emerald-400 p-3 rounded-2xl border border-emerald-800/50 text-[10px] uppercase flex items-center justify-center gap-2"><Coins size={14} /> Saldo</button>
          <button onClick={() => { setEditingCardId(null); setIsCardModalOpen(true); }} className="flex-1 bg-slate-800/50 text-slate-300 p-3 rounded-2xl border border-slate-700 text-[10px] uppercase flex items-center justify-center gap-2"><CreditCard size={14} /> Cartão</button>
          <button onClick={() => setIsModalOpen(true)} className="w-full md:w-auto bg-blue-600 text-white p-3.5 rounded-2xl shadow-lg text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-blue-700 transition-all"><Plus size={18} /> Novo Gasto</button>
        </div>
      </header>
        ...
        <div className="relative">
          <button onClick={() => setIsFilterMenuOpen(!isFilterMenuOpen)} className="w-full bg-slate-900 p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl border-b-8 border-amber-500 flex flex-col justify-between h-32 md:h-36 text-left active:scale-95 transition-all">
            ...
            <div className="flex items-center justify-between w-full">
              <div className="text-sm md:text-2xl font-black leading-normal tracking-tighter truncate uppercase italic px-1">{filtroCartao}</div>
          ...
          {isFilterMenuOpen && (
            <div className="absolute bottom-full left-0 right-0 mb-2 bg-slate-900 border-2 border-slate-800 rounded-3xl shadow-2xl z-[100] overflow-hidden animate-in fade-in slide-in-from-bottom-2">
              <div className="p-2 max-h-60 overflow-y-auto">
                ...
                {cartoes.map(c => (
                  <button key={c.id} onClick={() => { setFiltroCartao(c.banco); setIsFilterMenuOpen(false); }} className="w-full text-left p-4 hover:bg-slate-800 rounded-2xl border-t border-slate-800/50 uppercase font-black text-[10px] text-blue-400">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        <div className="lg:col-span-2 space-y-4 md:space-y-6">
          <div className="bg-slate-900 p-4 md:p-8 rounded-[2rem] border border-slate-800 shadow-2xl overflow-hidden">
            <h2 className="text-white font-black mb-4 uppercase text-[10px] tracking-widest leading-normal">Estatísticas</h2>
          ...
          <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 shadow-2xl">
            <h2 className="text-white font-black mb-4 uppercase text-[10px] tracking-widest leading-normal">Meus Cartões</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 font-black">
              {cartoes.map(c => (
                <div key={c.id} className="p-4 md:p-5 border-2 border-slate-800 rounded-2xl flex justify-between items-center bg-slate-950/50 hover:border-blue-500 transition-all">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-slate-800 rounded-xl flex items-center justify-center border border-slate-700 p-1.5 overflow-hidden flex-shrink-0">
                    ...
                    <div className="leading-normal">
                      ...
                      <p className="font-black text-xs uppercase italic px-1 leading-normal mb-1">{c.nome_cartao}</p>
        ...
        <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 h-[450px] md:h-[600px] overflow-hidden flex flex-col shadow-2xl">
          <h2 className="text-white font-black mb-4 uppercase text-[10px] tracking-widest leading-normal">Lançamentos</h2>
            ...
            {transacoesFiltradas.length > 0 ? transacoesFiltradas.map((t) => (
              <div key={t.id} className="flex justify-between items-center p-4 bg-slate-800/40 rounded-2xl border border-slate-800 hover:bg-slate-800/60 transition-all leading-normal">
                <div className="flex-1 min-w-0 mr-3">
                  <p className="font-black text-slate-200 text-[10px] uppercase tracking-tight truncate leading-normal mb-1">{t.descricao}</p>
                  <p className="text-[8px] text-slate-500 font-bold uppercase">{t.data_ordenacao}</p>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0 leading-normal">
                  <span className="font-black text-xs text-rose-500 whitespace-nowrap italic px-1">R$ {formatarMoeda(t.valor)}</span>
      ...
      {isAdminMenuOpen && (
        <div className="fixed inset-0 bg-black/10 backdrop-blur-xl flex items-center justify-center p-4 z-[500] animate-in fade-in duration-300 italic">
          <div className="bg-slate-900 w-full max-w-lg rounded-[3rem] border-4 border-amber-500/30 shadow-2xl overflow-hidden leading-normal">
            ...
            <div className="p-8">
              <div className="space-y-4 flex flex-col items-center justify-center p-8 bg-slate-800/50 rounded-[2.5rem] border-2 border-amber-500/10 italic">
                 <Users size={48} className="text-amber-500 mb-2 opacity-50" />
                 <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-normal">Total Usuários</span>
                 <div className="text-7xl font-black text-white tracking-tighter py-2 leading-normal italic px-1">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/10 backdrop-blur-md flex items-end md:items-center justify-center p-0 md:p-4 z-[200]">
          <form onSubmit={handleSalvarGasto} className="bg-slate-900 w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-6 md:p-10 border-t-4 md:border-4 border-slate-800 shadow-2xl overflow-y-auto max-h-[95vh] text-white">
            ...
            <div className="space-y-4 font-bold">
              <input value={descricao} onChange={(e) => setDescricao(e.target.value)} placeholder="DESCRIÇÃO" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white uppercase text-xs outline-none focus:border-blue-600 transition-all" required />
              <div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-500 font-black text-sm italic px-1">R$</span><input type="text" value={valorDisplay} onChange={(e) => setValorDisplay(aplicarMascara(e.target.value))} placeholder="0,00" className="w-full pl-10 p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 font-black text-blue-400 text-lg outline-none focus:border-blue-600" required /></div>
              <select value={formaPagamento} onChange={(e) => setFormaPagamento(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white uppercase text-xs outline-none focus:border-blue-600">
                <option value="Pix">Pix / Dinheiro</option>
                {cartoes.map(c => (<option key={c.id} value={`${c.banco} - ${c.nome_cartao}`}>{c.banco} ({c.nome_cartao})</option>))}
              </select>
              <input type="date" value={data} onChange={(e) => setData(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 font-bold text-white outline-none focus:border-blue-600" required />
              <button type="submit" className="w-full bg-blue-600 text-white font-black py-5 rounded-[2rem] shadow-xl hover:bg-blue-700 transition-all uppercase text-sm mt-2">Lançar Agora</button>
      ...
      {isSaldoModalOpen && (
        <div className="fixed inset-0 bg-black/10 backdrop-blur-md flex items-center justify-center p-4 z-[200]">
          <div className="bg-slate-900 w-full max-w-sm rounded-[2.5rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <button onClick={() => { const v = Number(saldoDisplay.replace(/\./g, '').replace(',', '.')); setSaldoInicial(v); localStorage.setItem(`@jfinancas:saldo:${user.id}`, v.toString()); setIsSaldoModalOpen(false); setSaldoDisplay(''); showAlert("Saldo ajustado!", 'success'); }} className="w-full bg-emerald-600 text-white font-black py-5 rounded-3xl uppercase text-xs active:scale-95 transition-all">Confirmar</button>
            <button type="button" onClick={() => setIsSaldoModalOpen(false)} className="w-full text-slate-500 font-bold uppercase text-[9px] mt-4 text-center">Fechar</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/10 backdrop-blur-md flex items-center justify-center p-4 z-[200]">
          <form onSubmit={handleSalvarCartao} className="bg-slate-900 w-full max-w-sm rounded-[2.5rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4 font-black">
              <input value={banco} onChange={(e) => setBanco(e.target.value)} placeholder="BANCO (EX: NUBANK)" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white uppercase text-xs outline-none focus:border-blue-600 transition-all" required />
              <div className="space-y-1">
                <input value={nomeCartao} onChange={(e) => handleNomeCartaoChange(e.target.value)} placeholder="APELIDO DO CARTÃO" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white uppercase text-xs outline-none focus:border-blue-600 transition-all" required />
                <p className="text-[7px] text-slate-500 ml-2 uppercase font-bold italic px-1">Máximo 12 letras.</p>
              </div>
              <input type="number" min="1" max="31" value={vencimento} onChange={(e) => setVencimento(e.target.value)} placeholder="DIA VENCIMENTO" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white font-bold text-xs outline-none focus:border-blue-600 transition-all" required />
              <button type="submit" className="w-full bg-blue-600 text-white font-black py-5 rounded-3xl uppercase text-xs mt-2">Confirmar</button>
...
function Card({ title, value, icon, color }: any) {
  return (
    <div className={`${color} p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl transition-transform active:scale-[0.98] border-black/20 flex flex-col justify-between h-32 md:h-36 text-white text-left`}>
      ...
      <div className="text-sm md:text-2xl font-black leading-normal tracking-tighter truncate italic px-1 uppercase">{value}</div>
    </div>
  );
}