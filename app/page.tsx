...
import { 
  ...
  UserCircle, ShieldCheck, Loader2, Filter, ChevronDown, Settings, 
  AlertCircle, CheckCircle, Users, Eye, EyeOff, Palette, Calendar, RefreshCcw
} from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="font-black antialiased">
      ...
      {view === 'auth' ? (
        <AuthScreen theme={theme} setUser={setUser} setView={setView} showAlert={showAlert} />
      ) : (
        <Dashboard user={user} theme={theme} currentThemeName={currentTheme} setTheme={setCurrentTheme} onLogout={handleLogout} showAlert={showAlert} />
      )}
    </div>
  );
}
...
function AuthScreen({ theme, setUser, setView, showAlert }: any) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (authMode === 'login') {
      ...
      if (error) return showAlert("Acesso negado. Verifique os dados.", 'error');
      setUser(data.user);
      setView('dashboard');
    } else {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-955 text-white">
      <form onSubmit={handleAuth} className="bg-slate-900 w-full max-w-md rounded-[2.5rem] p-8 md:p-10 border-4 border-slate-800 shadow-2xl">
        ...
        <div className="space-y-4">
          {authMode === 'signup' && <input type="text" placeholder="Nome completo" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none focus:border-blue-600 text-sm text-white" required />}
          <input type="email" placeholder="E-mail" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none focus:border-blue-600 text-sm text-white" required />
          <div className="relative">
            <input type={showPassword ? "text" : "password"} placeholder="Senha" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none focus:border-blue-600 text-sm text-white" required />
...
function Dashboard({ user, theme, currentThemeName, setTheme, onLogout, showAlert }: any) {
  ...
  const handleSalvarGasto = async (e: React.FormEvent) => {
    ...
    const valorFinal = tipo === 'despesa' ? -Math.abs(vTotal) : Math.abs(vTotal);
    ...
    for (let i = 0; i < parcelas; i++) {
      ...
      novosLancamentos.push({
        ...
        data_ordenacao: dataLancamento.toISOString().split('T')[0],
        user_id: user.id
      });
    }

    const { error } = await supabase.from('transacoes').insert(novosLancamentos);
    if (!error) { fetchDados(); setIsModalOpen(false); setDescricao(''); setValorDisplay(''); setParcelas(1); setRecorrente(false); showAlert("Lançado com sucesso!", 'success'); }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-955 p-2 md:p-8 text-white font-sans overflow-x-hidden pb-24 font-black">
      <header className="flex flex-col gap-4 mb-6 bg-slate-900 p-4 md:p-6 rounded-[2rem] border border-slate-800 shadow-2xl relative">
        <div className="flex justify-between items-center w-full">
          ...
          <div className="flex gap-2">
            ...
            <div className="relative">
              ...
              {isProfileMenuOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-slate-900 border-2 border-slate-800 rounded-[2rem] shadow-2xl z-[500] overflow-hidden animate-in fade-in slide-in-from-top-2">
                  ...
                  <button onClick={onLogout} className="w-full flex items-center gap-3 p-4 hover:bg-rose-900/20 text-rose-500 rounded-2xl transition-all uppercase text-[10px]"><LogOut size={18} /> Sair do App</button>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { setEditingCardId(null); setBanco(''); setNomeCartao(''); setVencimento(''); setIsCardModalOpen(true); }} className="flex-1 bg-slate-800/50 text-slate-300 p-3 rounded-2xl border border-slate-700 text-[10px] uppercase flex items-center justify-center gap-2 active:scale-95 italic"><CreditCard size={14} /> Cartão</button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6">
        <Card title="Saldo Atual" value={`R$ ${formatarMoeda(saldoFinal)}`} icon={<Banknote size={20}/>} color={`bg-slate-900 border-b-8 ${theme.border}`} />
        <Card title="Gasto Mensal" value={`R$ ${formatarMoeda(saidas)}`} icon={<CreditCard size={20}/>} color="bg-slate-900 border-b-8 border-rose-600" />
        <Card title="Entradas" value={`R$ ${formatarMoeda(entradas)}`} icon={<TrendingUp size={20}/>} color="bg-slate-900 border-b-8 border-emerald-600" />
        <div className="relative">
          <button onClick={() => setIsFilterMenuOpen(!isFilterMenuOpen)} className="w-full bg-slate-900 p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl border-b-8 border-amber-500 flex flex-col justify-between h-32 md:h-36 text-left active:scale-95">
            <span className="text-white/40 font-black text-[7px] md:text-[10px] uppercase tracking-widest leading-normal">Filtrar:</span>
            <div className="flex items-center justify-between w-full leading-normal">
              <div className="text-sm md:text-xl font-black truncate uppercase italic px-1">{filtroCartao}</div>
              <ChevronDown size={16} className={`text-amber-500 transition-transform ${isFilterMenuOpen ? 'rotate-180' : ''}`} />
            </div>
          </button>
          {isFilterMenuOpen && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900 border-2 border-slate-800 rounded-3xl shadow-2xl z-[400] overflow-hidden animate-in fade-in slide-in-from-top-2 font-black italic">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 shadow-2xl font-black">
              ...
              {cartoes.map(c => (
                <div key={c.id} className="p-4 border-2 border-slate-800 rounded-2xl flex justify-between items-center bg-slate-950/50 hover:border-blue-500 transition-all">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="leading-tight italic">
                      ...
                      <p className={`text-[9px] font-bold ${theme.text} uppercase`}>DIA {c.vencimento}</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
        ...
        <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 h-full overflow-hidden flex flex-col shadow-2xl min-h-[500px] italic">
          ...
          <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar flex-1 font-black">
            {transacoesFiltradas.map((t) => (
              <div key={t.id} className="flex justify-between items-center p-4 bg-slate-800/40 rounded-2xl border border-slate-800 hover:border-slate-600 transition-all">
                <div className="flex-1 min-w-0 mr-3 leading-tight">
                  <div className="flex items-center gap-2">
                    <p className="text-slate-200 text-[10px] uppercase truncate">{t.descricao}</p>
                    {t.recorrente && <RefreshCcw size={10} className="text-blue-400" />}
                  </div>
                  <p className="text-[8px] text-slate-500 uppercase">{t.data_ordenacao}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-xs px-1 ${t.valor > 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                    {t.valor > 0 ? '+' : '-'} R$ {formatarMoeda(t.valor)}
                  </span>
      ...
      {/* MODAL NOVO GASTO (COMPLETO) */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[3000] animate-in fade-in zoom-in-95 duration-300">
          <form onSubmit={handleSalvarGasto} className="bg-slate-900 w-full max-w-md rounded-[3rem] p-6 md:p-8 border-4 border-slate-800 shadow-2xl text-white font-black">
            <div className="flex justify-between items-center mb-6 px-1">
               <h2 className="text-xl uppercase tracking-widest italic">Novo Lançamento</h2>
            ...
            <div className="space-y-4">
              <div className="flex gap-2 p-1 bg-slate-800 rounded-2xl">
                <button type="button" onClick={() => setTipo('despesa')} className={`flex-1 py-3 rounded-xl text-[10px] uppercase transition-all ${tipo === 'despesa' ? 'bg-rose-600 text-white' : 'text-slate-500'}`}>Despesa</button>
                <button type="button" onClick={() => setTipo('receita')} className={`flex-1 py-3 rounded-xl text-[10px] uppercase transition-all ${tipo === 'receita' ? 'bg-emerald-600 text-white' : 'text-slate-500'}`}>Receita</button>
              </div>
              <input value={descricao} onChange={(e) => setDescricao(e.target.value)} placeholder="O que é?" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-sm uppercase" required />
              <div className="relative">
                <span className={`absolute left-4 top-1/2 -translate-y-1/2 ${tipo === 'receita' ? 'text-emerald-500' : 'text-rose-500'} text-sm px-1 italic font-black`}>R$</span>
                <input type="text" value={valorDisplay} onChange={(e) => setValorDisplay(aplicarMascara(e.target.value))} placeholder="0,00" className={`w-full pl-10 p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 ${tipo === 'receita' ? 'text-emerald-400' : 'text-rose-400'} text-lg outline-none italic`} required />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <select value={formaPagamento} onChange={(e) => setFormaPagamento(e.target.value)} className="p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-xs outline-none">
                  <option value="Pix">Pix / Dinheiro</option>
                  {cartoes.map(c => (<option key={c.id} value={`${c.banco} (${c.nome_cartao})`}>{c.banco}</option>))}
                </select>
                <input type="date" value={data} onChange={(e) => setData(e.target.value)} className="p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-xs outline-none text-center" required />
              </div>
              <div className="p-4 bg-slate-950/50 rounded-2xl border border-slate-800 space-y-3">
                <div className="flex items-center justify-between"><label className="text-[10px] uppercase text-slate-500 flex items-center gap-2"><Calendar size={14}/> Parcelas</label><input type="number" min="1" max="12" value={parcelas} onChange={(e) => setParcelas(Number(e.target.value))} className="w-16 bg-slate-800 border border-slate-700 rounded-lg p-1 text-center text-xs" /></div>
                <div className="flex items-center justify-between"><label className="text-[10px] uppercase text-slate-500 flex items-center gap-2"><RefreshCcw size={14}/> Recorrente?</label><button type="button" onClick={() => setRecorrente(!recorrente)} className={`w-10 h-5 rounded-full relative transition-all ${recorrente ? 'bg-emerald-600' : 'bg-slate-700'}`}><div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all ${recorrente ? 'left-5.5' : 'left-0.5'}`} /></button></div>
              </div>
              <button type="submit" className={`w-full ${theme.primary} text-white py-5 rounded-[2rem] shadow-xl uppercase text-xs mt-2 italic leading-none`}>Finalizar</button>
            </div>
          </form>
        </div>
      )}
      ...
      {isConfigModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[3000] animate-in fade-in zoom-in-95 duration-300 font-black">
          <form onSubmit={handleUpdateProfile} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-10 border-4 border-slate-800 shadow-2xl text-white">
            <div className="flex justify-between items-center mb-8 px-1"><h2 className="text-xl uppercase tracking-widest italic">Ajustes</h2><button type="button" onClick={() => setIsConfigModalOpen(false)} className="bg-slate-800 p-2 rounded-full text-slate-500"><X size={20} /></button></div>
            <div className="mb-8"><p className="text-[8px] text-slate-500 uppercase mb-4 tracking-widest flex items-center gap-2"><Palette size={12}/> Estilo</p><div className="flex justify-between px-2">{Object.keys(THEMES).map((tName) => <button key={tName} type="button" onClick={() => changeTheme(tName as any)} className={`w-10 h-10 rounded-full border-4 ${currentThemeName === tName ? 'border-white scale-110 shadow-2xl' : 'border-transparent opacity-40'} ${THEMES[tName as keyof typeof THEMES].primary} transition-all`} />)}</div></div>
            <div className="space-y-4"><p className="text-[8px] text-slate-500 uppercase tracking-widest flex items-center gap-2"><UserCircle size={12}/> Meus Dados</p><input value={novoNome} onChange={(e) => setNovoNome(e.target.value)} placeholder="Nome" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-white text-sm" /><input type="password" value={novaSenha} onChange={(e) => setNovaSenha(e.target.value)} placeholder="Nova senha" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white text-sm" /><button type="submit" className={`w-full ${theme.primary} py-5 rounded-[2rem] uppercase text-xs mt-2 font-black shadow-lg italic leading-none`}>Salvar</button></div>
          </form>
        </div>
      )}
      ...
      {/* MODAL SALDO */}
      {isSaldoModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[3000] animate-in fade-in zoom-in-95 font-black italic">
          <div className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-10 border-4 border-slate-800 shadow-2xl text-white">
            <h2 className="text-xl mb-8 text-emerald-500 text-center uppercase tracking-widest leading-none">Saldo Inicial</h2>
            <div className="relative mb-6"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-600 text-lg px-1 font-black">R$</span><input type="text" value={saldoDisplay} onChange={(e) => setSaldoDisplay(aplicarMascara(e.target.value))} placeholder="0,00" className="w-full pl-12 p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-emerald-500 text-xl outline-none font-black" /></div>
            <button onClick={handleConfirmarSaldo} className="w-full bg-emerald-600 py-5 rounded-[2rem] uppercase text-xs shadow-lg active:scale-95 leading-none italic">Confirmar</button>
            <button onClick={() => setIsSaldoModalOpen(false)} className="w-full text-slate-500 py-4 mt-2 uppercase text-[9px] leading-none">Fechar</button>
          </div>
        </div>
      )}

      {/* MODAL CARTÃO (COM TRAVAS) */}
      {isCardModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[3000] animate-in fade-in zoom-in-95 font-black italic">
          <form onSubmit={handleSalvarCartao} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-8 border-4 border-slate-800 shadow-2xl text-white">
            <h2 className="text-xl mb-8 text-center uppercase tracking-widest leading-none">{editingCardId ? 'Editar' : 'Novo'} Cartão</h2>
            <div className="space-y-4">
              <input value={banco} onChange={(e) => setBanco(e.target.value)} placeholder="Banco (ex: Nubank)" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-sm uppercase" required />
              <div><input value={nomeCartao} onChange={(e) => {
                const val = e.target.value.replace(/[0-9]/g, '');
                if (val.length <= 12) setNomeCartao(val);
              }} placeholder="Apelido" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-sm uppercase" required /><p className="text-[7px] text-slate-500 mt-1 uppercase italic px-1">Máximo 12 letras.</p></div>
              ...
              <button type="submit" className={`w-full ${theme.primary} py-5 rounded-[2rem] uppercase text-xs mt-4 active:scale-95 leading-none`}>Salvar Cartão</button>
              <button type="button" onClick={() => setIsCardModalOpen(false)} className="w-full text-slate-500 py-4 mt-2 uppercase text-[9px] leading-none">Cancelar</button>
            </div>
          </form>
        </div>
      )}
      ...
      {/* MODAL ADMIN */}
      {isAdminMenuOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[4000] animate-in fade-in duration-300 font-black">
          <div className="bg-slate-900 w-full max-w-sm rounded-[3rem] border-4 border-amber-500/30 shadow-2xl overflow-hidden font-black italic">
            <div className="bg-amber-500 p-6 flex justify-between items-center text-slate-950"><div><h2 className="uppercase tracking-tighter text-xl px-1 leading-none">Admin Panel</h2><p className="text-[10px] uppercase mt-1 leading-none">Global</p></div><button onClick={() => setIsAdminMenuOpen(false)} className="bg-slate-950/20 p-2 rounded-full leading-none"><X size={24} /></button></div>
            <div className="p-8 flex flex-col items-center justify-center bg-slate-800/50 m-4 rounded-[2.5rem] border-2 border-amber-500/10 italic font-black">
                 <Users size={48} className="text-amber-500 mb-2 opacity-50" /><span className="text-[10px] text-slate-500 uppercase tracking-widest leading-none">Total Usuários</span>
                 <div className="text-7xl text-white tracking-tighter py-4 px-1 leading-none">{totalUsuarios}</div>
                 <p className="text-[7px] text-amber-500/40 uppercase mt-2 tracking-widest">Base Supabase</p>
            </div>
          </div>
        </div>
      )}
