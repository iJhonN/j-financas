...
export default function App() {
  ...
  return (
    <>
      ...
      {view === 'auth' ? (
        <AuthScreen theme={THEMES[theme]} setView={setView} setUser={setUser} showAlert={showAlert} />
      ) : (
        <Dashboard user={user} theme={THEMES[theme]} currentThemeName={theme} setTheme={setTheme} onLogout={handleLogout} showAlert={showAlert} />
      )}
    </>
  );
}
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-955 text-white font-black">
      <form onSubmit={handleAuth} className="bg-slate-900 w-full max-w-md rounded-[2.5rem] p-8 md:p-10 border-4 border-slate-800 shadow-2xl">
        ...
        <div className="space-y-4">
          {authMode === 'signup' && (
            <input type="text" placeholder="Nome completo" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none focus:border-blue-600 text-sm" required />
          )}
          <input type="email" placeholder="E-mail" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none focus:border-blue-600 text-sm" required />
          <div className="relative">
            <input type={showPassword ? "text" : "password"} placeholder="Senha" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none focus:border-blue-600 text-sm" required />
...
function Dashboard({ user, theme, currentThemeName, setTheme, onLogout, showAlert }: any) {
  ...
  const handleSalvarGasto = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('transacoes').insert([{ descricao: cleanDesc.toUpperCase(), valor: vTotal, forma_pagamento: formaPagamento, data_ordenacao: data, user_id: user.id }]);
    if (!error) { fetchDados(); setIsModalOpen(false); setDescricao(''); setValorDisplay(''); showAlert("Lançado!", 'success'); }
  };
    ...
    <div className="min-h-screen bg-slate-955 p-2 md:p-8 text-white font-sans overflow-x-hidden pb-20 font-black">
      <header className="flex flex-col gap-4 mb-6 bg-slate-900 p-4 md:p-6 rounded-[2rem] border border-slate-800 shadow-2xl relative">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setIsModalOpen(true)} className={`w-full md:w-auto ${theme.primary} text-white p-3.5 rounded-2xl shadow-lg text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 ${theme.hover} active:scale-95 italic`}><Plus size={18} /> Novo Gasto</button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6">
        ...
        <div className="relative">
          ...
          {isFilterMenuOpen && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900 border-2 border-slate-800 rounded-3xl shadow-2xl z-[500] overflow-hidden animate-in fade-in slide-in-from-top-2 font-black">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {cartoes.map(c => (
                <div key={c.id} className="p-4 md:p-5 border-2 border-slate-800 rounded-2xl flex justify-between items-center bg-slate-950/50 hover:border-blue-500 transition-all group font-black">
                  <div className="flex items-center gap-3">
        ...
        <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 h-full overflow-hidden flex flex-col shadow-2xl min-h-[500px]">
          ...
          <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar font-black flex-1">
            {transacoesFiltradas.length > 0 ? transacoesFiltradas.map((t) => (
              <div key={t.id} className="flex justify-between items-center p-4 bg-slate-800/40 rounded-2xl border border-slate-800 hover:bg-slate-800/60 transition-all leading-normal">
                <div className="flex-1 min-w-0 mr-3">
                  <p className="font-black text-slate-200 text-[10px] uppercase truncate mb-1">{t.descricao}</p>
                  <p className="text-[8px] text-slate-500 font-bold uppercase">{t.data_ordenacao}</p>
                </div>
                <div className="flex items-center gap-2 italic">
                  <span className="font-black text-xs text-rose-500 px-1 whitespace-nowrap leading-tight">R$ {formatarMoeda(t.valor)}</span>
      ...
      {/* MODAIS RESTAURADOS (CONFIG, GASTO, SALDO, CARTÃO, ADMIN) */}
      {isConfigModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[2000] animate-in fade-in zoom-in-95 duration-300 font-black">
          <form onSubmit={handleUpdateProfile} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4">
              ...
              <button type="submit" className={`w-full ${theme.primary} py-5 rounded-[2rem] uppercase text-xs mt-2 font-black shadow-lg ${theme.shadow} leading-normal italic`}>Salvar Alterações</button>
            </div>
          </form>
        </div>
      )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1000] animate-in fade-in zoom-in-95 duration-300 font-black">
          <form onSubmit={handleSalvarGasto} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            <div className="flex justify-between items-center mb-8 px-1">
               <h2 className="text-xl uppercase tracking-widest italic">Novo Gasto</h2>
            ...
            <div className="space-y-4">
              ...
              <select value={formaPagamento} onChange={(e) => setFormaPagamento(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white text-sm outline-none">
              ...
              <div className="flex justify-center font-black">
                <input type="date" value={data} onChange={(e) => setData(e.target.value)} className="w-auto p-3 bg-slate-800 rounded-xl border-2 border-slate-700 text-white text-sm outline-none text-center" required />
              </div>
              <button type="submit" className={`w-full ${theme.primary} text-white py-5 rounded-[2rem] shadow-xl ${theme.hover} active:scale-95 transition-all uppercase text-xs mt-2 italic`}>Lançar Agora</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1000] animate-in fade-in duration-300 font-black italic">
          <form onSubmit={handleSalvarCartao} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4 font-black">
              ...
              <button type="button" onClick={() => setIsCardModalOpen(false)} className="w-full text-slate-500 py-4 mt-2 uppercase text-[9px] font-black leading-normal italic">Cancelar</button>
            </div>
          </form>
        </div>
      )}
