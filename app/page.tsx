...
// Sistema de Temas Dinâmicos (Sincronizados)
const THEMES = {
...
export default function App() {
  ...
  return (
    <>
      {alertConfig.show && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[3000] animate-in fade-in slide-in-from-top-4 duration-300 px-4 w-full max-w-sm font-black">
      ...
      {view === 'auth' ? (
        ...
        <Dashboard user={user} theme={theme} setTheme={setTheme} onLogout={handleLogout} showAlert={showAlert} />
      )}
    </>
  );
}

// --- TELA DE LOGIN (SEM CAIXA ALTA FORÇADA) ---
function AuthScreen({ theme, setUser, setView, showAlert }: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-955 text-white font-black">
      <form onSubmit={handleAuth} className="bg-slate-900 w-full max-w-md rounded-[2.5rem] p-8 md:p-10 border-4 border-slate-800 shadow-2xl">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-xl md:text-2xl font-black uppercase tracking-tighter italic px-1">J FINANÇAS</h1>
        </div>
        <div className="space-y-4 font-normal">
          {authMode === 'signup' && <input type="text" placeholder="Nome completo" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none focus:border-blue-600 text-white" required />}
          <input type="email" placeholder="E-mail" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none focus:border-blue-600 text-white" required />
          <div className="relative">
            <input type={showPassword ? "text" : "password"} placeholder="Senha" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none focus:border-blue-600 text-white" required />
          ...
          <button type="submit" className={`w-full ${theme.primary} text-white font-black py-5 rounded-[2rem] shadow-xl hover:bg-blue-700 transition-all uppercase text-sm mt-4 italic`}>{authMode === 'login' ? 'Entrar' : 'Cadastrar'}</button>
...
function Dashboard({ user, theme: currentThemeName, setTheme, onLogout, showAlert }: any) {
  const theme = THEMES[currentThemeName as keyof typeof THEMES];
  ...
  const fetchDados = async () => {
    ...
    // SYNC: Busca saldo e tema do banco de dados
    const { data: profile } = await supabase.from('profiles').select('saldo_inicial, theme').eq('id', user.id).single();
    if (profile) {
  ...
  const handleConfirmarSaldo = async () => {
    ...
    if (!error) {
      ...
      setSaldoDisplay('');
      showAlert("Saldo sincronizado!", "success");
    }
  };
  ...
  const handleSalvarGasto = async (e: React.FormEvent) => {
    e.preventDefault();
    const descLimpa = descricao.replace(/<[^>]*>?/gm, '').trim();
    if (!descLimpa) return showAlert("Descrição inválida", "error");
    const vTotal = Number(valorDisplay.replace(/\./g, '').replace(',', '.'));
    await supabase.from('transacoes').insert([{ descricao: descLimpa.toUpperCase(), valor: vTotal, forma_pagamento: formaPagamento, data_ordenacao: data, user_id: user.id }]);
  ...
  return (
    <div className="min-h-screen bg-slate-955 p-2 md:p-8 text-white font-sans overflow-x-hidden pb-20 font-black">
          ...
          <div className="flex gap-2 leading-normal">
            ...
            <div className="relative">
              <button onClick={() => setIsProfileMenuOpen(!isProfileMenuOpen)} className="bg-slate-800 text-slate-300 p-2.5 rounded-full border border-slate-700 hover:bg-blue-600 transition-all relative">
                <UserCircle size={20} />
                {notifPermission !== 'granted' && <span className="absolute top-0 right-0 w-2 h-2 bg-rose-500 rounded-full animate-pulse"></span>}
              </button>
              {isProfileMenuOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-slate-900 border-2 border-slate-800 rounded-[2rem] shadow-2xl z-[110] overflow-hidden animate-in fade-in slide-in-from-top-2">
                  <div className="p-2 space-y-1 font-black">
      ...
      {/* CARDS RESUMO */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6 leading-normal font-black italic">
        ...
        <div className="relative">
          <button onClick={() => setIsFilterMenuOpen(!isFilterMenuOpen)} className="w-full bg-slate-900 p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl border-b-8 border-amber-500 flex flex-col justify-between h-32 md:h-36 text-left active:scale-95 font-black italic">
            <span className="text-white/40 text-[7px] md:text-[10px] uppercase tracking-widest leading-normal">Filtrar por:</span>
            <div className="flex items-center justify-between w-full leading-normal">
              <div className="text-sm md:text-xl font-black truncate uppercase px-1 leading-tight">{filtroCartao}</div>
              <ChevronDown size={16} className={`text-amber-500 transition-transform ${isFilterMenuOpen ? 'rotate-180' : ''}`} />
            </div>
          </button>
          {isFilterMenuOpen && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900 border-2 border-slate-800 rounded-3xl shadow-2xl z-[500] overflow-hidden animate-in fade-in slide-in-from-top-2 font-black italic">
              <div className="p-2 max-h-64 overflow-y-auto uppercase text-[10px]">
                ...
                {cartoes.map(c => <button key={c.id} onClick={() => { setFiltroCartao(c.banco); setIsFilterMenuOpen(false); }} className={`w-full text-left p-4 hover:bg-slate-800 border-t border-slate-800/50 ${theme.text}`}>{c.banco} <br/><span className="text-[7px] text-slate-500 lowercase">{c.nome_cartao}</span></button>)}
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-slate-900 p-6 rounded-[2.5rem] border border-slate-800 shadow-2xl h-80 overflow-hidden">
          ...
          <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 shadow-2xl font-black italic">
            <h2 className="text-white font-black mb-6 uppercase text-[10px] tracking-widest italic leading-normal">Meus Cartões</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {cartoes.map(c => (
                <div key={c.id} className="p-4 md:p-5 border-2 border-slate-800 rounded-2xl flex justify-between items-center bg-slate-950/50 hover:border-blue-500 transition-all group leading-normal">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="leading-tight">
                      ...
                      <p className="font-black text-xs uppercase mb-1">{c.nome_cartao}</p>
                  ...
                  <div className="flex gap-2 leading-normal">
                    ...
                    <button onClick={async () => { if(confirm("Excluir?")) { await supabase.from('cartoes').delete().eq('id', c.id); fetchDados(); showAlert("Removido", 'success'); } }} className="text-slate-600 hover:text-rose-500 transition-all"><Trash2 size={16} /></button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 h-full overflow-hidden flex flex-col shadow-2xl min-h-[500px] leading-normal font-black italic">
          <h2 className="text-white font-black mb-4 uppercase text-[10px] tracking-widest italic leading-normal px-1">Lançamentos</h2>
          <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar flex-1">
            {transacoesFiltradas.length > 0 ? transacoesFiltradas.map((t) => (
              <div key={t.id} className="flex justify-between items-center p-4 bg-slate-800/40 rounded-2xl border border-slate-800 hover:bg-slate-800/60 transition-all leading-normal font-black">
                <div className="flex-1 min-w-0 mr-3 leading-normal font-black">
                  <p className="font-black text-slate-200 text-[10px] uppercase truncate mb-1 leading-normal">{t.descricao}</p>
                  <p className="text-[8px] text-slate-500 font-bold uppercase">{t.data_ordenacao}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-black text-xs text-rose-500 px-1 whitespace-nowrap leading-tight italic">R$ {formatarMoeda(t.valor)}</span>
                  <button onClick={async () => { if(confirm("Apagar gasto?")) { await supabase.from('transacoes').delete().eq('id', t.id); fetchDados(); showAlert("Removido", 'success'); } }} className="text-slate-700 hover:text-rose-500 transition-all"><Trash2 size={14} /></button>
                </div>
              </div>
      ...
      {isConfigModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[2000] animate-in fade-in zoom-in-95 duration-300 font-black">
          <form onSubmit={handleUpdateProfile} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-10 border-4 border-slate-800 shadow-2xl text-white">
            <div className="flex justify-between items-center mb-8 px-1">
               <h2 className="text-xl uppercase tracking-widest italic">Ajustes</h2>
            ...
            <div className="mb-8">
              <p className="text-[8px] text-slate-500 uppercase mb-4 tracking-widest flex items-center gap-2"><Palette size={12}/> Escolher Cor do App</p>
            ...
            <div className="space-y-4 leading-normal">
              ...
              <input value={novoNome} onChange={(e) => setNovoNome(e.target.value)} placeholder="Nome" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-white text-sm" />
              <input type="password" value={novaSenha} onChange={(e) => setNovaSenha(e.target.value)} placeholder="Nova senha (opcional)" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white text-sm" />
              <button type="submit" className={`w-full ${theme.primary} py-5 rounded-[2rem] uppercase text-xs mt-2 font-black shadow-lg ${theme.shadow} italic leading-normal`}>Salvar Alterações</button>
            </div>
          </form>
        </div>
      )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1000] animate-in fade-in zoom-in-95 duration-300 font-black">
          <form onSubmit={handleSalvarGasto} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            <div className="flex justify-between items-center mb-8 px-1 font-black italic leading-normal">
            ...
            <div className="space-y-4">
              <input value={descricao} onChange={(e) => setDescricao(e.target.value)} placeholder="Descrição" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-white text-sm uppercase" required />
              <div className="relative leading-normal">
                <span className={`absolute left-4 top-1/2 -translate-y-1/2 ${theme.text} text-sm px-1 italic font-black`}>R$</span>
                <input type="text" value={valorDisplay} onChange={(e) => setValorDisplay(aplicarMascara(e.target.value))} placeholder="0,00" className={`w-full pl-10 p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 ${theme.text} text-lg outline-none font-black italic`} required />
              </div>
              <select value={formaPagamento} onChange={(e) => setFormaPagamento(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white text-sm outline-none font-black">
              ...
              <div className="flex justify-center leading-normal">
                <input type="date" value={data} onChange={(e) => setData(e.target.value)} className="w-auto p-3 bg-slate-800 rounded-xl border-2 border-slate-700 text-white text-sm outline-none text-center font-black" required />
              </div>
      ...
      {isSaldoModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1000] animate-in fade-in duration-300 font-black italic leading-normal">
          <div className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <button onClick={handleConfirmarSaldo} className="w-full bg-emerald-600 py-5 rounded-[2rem] uppercase text-xs shadow-lg active:scale-95 font-black leading-normal italic">Confirmar</button>
        ...
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1000] animate-in fade-in duration-300 font-black italic leading-normal">
          <form onSubmit={handleSalvarCartao} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4 font-black">
              <input value={banco} onChange={(e) => setBanco(e.target.value)} placeholder="Banco (ex: Inter)" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-sm uppercase" required />
              <div>
                <input value={nomeCartao} onChange={(e) => handleNomeCartaoChange(e.target.value)} placeholder="Apelido do cartão" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-sm uppercase" required />
                <p className="text-[7px] text-slate-500 ml-2 mt-1 uppercase italic leading-tight px-1 font-black">Máximo 12 caracteres.</p>
              </div>
              ...
              <button type="button" onClick={() => setIsCardModalOpen(false)} className="w-full text-slate-500 py-4 mt-2 uppercase text-[9px] font-black italic leading-normal">Cancelar</button>
            </div>
          </form>
        </div>
      )}
      ...
      {/* MODAL ADMIN */}
      {isAdminMenuOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1500] animate-in fade-in duration-300 font-black italic leading-normal">
          <div className="bg-slate-900 w-full max-w-sm rounded-[3rem] border-4 border-amber-500/30 shadow-2xl overflow-hidden">
            <div className="bg-amber-500 p-6 flex justify-between items-center text-slate-950">
              <div><h2 className="uppercase tracking-tighter text-xl px-1 italic font-black leading-normal">Admin Panel</h2><p className="text-[10px] uppercase mt-1 leading-normal font-black">Status Global</p></div>
              <button onClick={() => setIsAdminMenuOpen(false)} className="bg-slate-950/20 p-2 rounded-full leading-normal"><X size={24} /></button>
            </div>
            <div className="p-8 flex flex-col items-center justify-center bg-slate-800/50 m-4 rounded-[2.5rem] border-2 border-amber-500/10 font-black italic leading-normal">
                 ...
                 <p className="text-[7px] text-amber-500/40 uppercase mt-2 tracking-widest leading-normal px-1">Base Supabase</p>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className={`${color} p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl transition-transform active:scale-[0.98] border-black/20 flex flex-col justify-between h-32 md:h-36 text-white text-left font-black italic leading-normal`}>
      <div className="flex justify-between items-start w-full">
        <span className="text-white/20 font-black text-[7px] md:text-[10px] uppercase tracking-widest italic">{title}</span>
