...
import { 
  ...
  UserCircle, ShieldCheck, Loader2, Filter, ChevronDown, Bell, BellOff, Settings, AlertCircle, CheckCircle, Users, Eye, EyeOff, Palette
} from 'lucide-react';
...
const THEMES = {
  blue: { primary: 'bg-blue-600', text: 'text-blue-400', border: 'border-blue-600', hover: 'hover:bg-blue-700', shadow: 'shadow-blue-500/20' },
  emerald: { primary: 'bg-emerald-600', text: 'text-emerald-400', border: 'border-emerald-600', hover: 'hover:bg-emerald-700', shadow: 'shadow-emerald-500/20' },
  purple: { primary: 'bg-violet-600', text: 'text-violet-400', border: 'border-violet-600', hover: 'hover:bg-violet-700', shadow: 'shadow-violet-500/20' },
  sunset: { primary: 'bg-rose-600', text: 'text-rose-400', border: 'border-rose-600', hover: 'hover:bg-rose-700', shadow: 'shadow-rose-500/20' }
};
...
export default function App() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (session) {
        ...
        setView('dashboard');
        // Recupera o tema salvo do usuário
  ...
  return (
    <>
      ...
      {view === 'auth' ? (
        <AuthScreen theme={THEMES[theme]} setView={setView} setUser={setUser} authMode={authMode} setAuthMode={setAuthMode} showAlert={showAlert} />
      ) : (
...
function AuthScreen({ theme, setView, setUser, authMode, setAuthMode, showAlert }: any) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (authMode === 'login') {
      ...
      if (error) return showAlert("Erro no login", 'error');
      setUser(data.user);
      setView('dashboard');
    } else {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-955 text-white font-black italic">
      <form onSubmit={handleAuth} className="bg-slate-900 w-full max-w-md rounded-[2.5rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl">
        ...
        <div className="space-y-4">
          {authMode === 'signup' && (
            <input type="text" placeholder="NOME" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-white focus:border-blue-600 uppercase text-[10px]" required />
          )}
          <input type="email" placeholder="E-MAIL" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-white focus:border-blue-600 uppercase text-[10px]" required />
          ...
          <button type="submit" className={`w-full ${theme.primary} text-white font-black py-5 rounded-[2rem] shadow-xl ${theme.hover} transition-all uppercase text-sm mt-4 tracking-widest leading-normal italic`}>
...
function Dashboard({ user, theme, currentThemeName, setTheme, onLogout, showAlert }: any) {
  ...
  const changeTheme = (newTheme: keyof typeof THEMES) => {
    ...
    showAlert(`Tema ${newTheme} aplicado!`, 'success');
  };
    ...
    <div className="min-h-screen bg-slate-955 p-2 md:p-8 text-white font-sans overflow-x-hidden pb-20">
      <header className="flex flex-col gap-4 mb-6 bg-slate-900 p-4 md:p-6 rounded-[2rem] border border-slate-800 shadow-2xl relative">
        <div className="flex justify-between items-center w-full">
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              ...
              <p className={`text-[9px] md:text-[10px] font-black ${theme.text} mt-1 uppercase leading-normal`}>Olá, {user?.user_metadata?.full_name?.split(' ')[0]}</p>
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <div className="relative">
              ...
              {isProfileMenuOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-slate-900 border-2 border-slate-800 rounded-[2rem] shadow-2xl z-[110] overflow-hidden animate-in fade-in slide-in-from-top-2 italic font-black">
                  <div className="p-3 space-y-1">
                    {/* SELETOR DE TEMAS NO PERFIL */}
                    <div className="p-4 border-b border-slate-800">
                      <p className="text-[8px] text-slate-500 uppercase mb-3 tracking-widest flex items-center gap-2"><Palette size={12}/> Escolher Tema</p>
                      <div className="flex justify-between">
                        {Object.keys(THEMES).map((tName) => (
                          <button 
                            key={tName} 
                            onClick={() => changeTheme(tName as any)}
                            className={`w-8 h-8 rounded-full border-2 ${currentThemeName === tName ? 'border-white scale-110' : 'border-transparent opacity-50'} ${THEMES[tName as keyof typeof THEMES].primary} transition-all`}
                          />
                        ))}
                      </div>
                    </div>
                    ...
                    <button onClick={onLogout} className="w-full flex items-center gap-3 p-4 hover:bg-rose-900/20 text-rose-500 rounded-2xl transition-all border-t border-slate-800/50 mt-1 uppercase text-[10px]">
                      <LogOut size={18} /> Sair
                    </button>
                  </div>
                </div>
        ...
        <div className="flex gap-2 font-black italic">
          ...
          <button onClick={() => { setEditingCardId(null); setIsCardModalOpen(true); }} className="flex-1 bg-slate-800/50 text-slate-300 p-3 rounded-2xl border border-slate-700 text-[10px] uppercase flex items-center justify-center gap-2 active:scale-95"><CreditCard size={14} /> Cartão</button>
          ...
          {isFilterMenuOpen && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900 border-2 border-slate-800 rounded-3xl shadow-2xl z-[100] overflow-hidden animate-in fade-in slide-in-from-top-2">
              <div className="p-2 max-h-64 overflow-y-auto font-black text-[10px] uppercase italic">
                ...
                {cartoes.map(c => (
                  <button key={c.id} onClick={() => { setFiltroCartao(c.banco); setIsFilterMenuOpen(false); }} className={`w-full text-left p-4 hover:bg-slate-800 rounded-2xl border-t border-slate-800/50 ${theme.text}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2 space-y-6">
            <div className="bg-slate-900 p-6 rounded-[2.5rem] border border-slate-800 shadow-2xl h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={[...transacoesFiltradas].reverse().map(t => ({ name: t.data_ordenacao, valor: t.valor }))}>
                    ...
                    <Area type="monotone" dataKey="valor" stroke={theme.text.includes('blue') ? '#3b82f6' : theme.text.includes('emerald') ? '#10b981' : theme.text.includes('violet') ? '#8b5cf6' : '#e11d48'} fill={theme.text.includes('blue') ? '#3b82f6' : theme.text.includes('emerald') ? '#10b981' : theme.text.includes('violet') ? '#8b5cf6' : '#e11d48'} fillOpacity={0.1} strokeWidth={4} />
                  </AreaChart>
                </ResponsiveContainer>
            </div>
            ...
            <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 shadow-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-black italic">
                  {cartoes.map(c => (
                    <div key={c.id} className={`p-4 md:p-5 border-2 border-slate-800 rounded-2xl flex justify-between items-center bg-slate-950/50 hover:${theme.border} transition-all`}>
                      <div className="flex items-center gap-3">
        ...
        <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 h-full overflow-hidden flex flex-col shadow-2xl">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[200] animate-in fade-in zoom-in-95 duration-300 font-black italic">
          <form onSubmit={handleSalvarGasto} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                <span className={`absolute left-4 top-1/2 -translate-y-1/2 ${theme.text} text-sm italic px-1`}>R$</span>
      ...
      {/* MODAIS RESTANTES (SALDO, CARTÃO, PERFIL, ADMIN) COM TEMAS... */}
      {isProfileEditModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[200] animate-in fade-in zoom-in-95 duration-300 font-black italic">
          <form onSubmit={handleUpdateProfile} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4">
              ...
              <button type="submit" className={`w-full ${theme.primary} py-5 rounded-[2rem] uppercase text-xs mt-2 font-black italic`}>Salvar Alterações</button>
        ...
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[200] animate-in fade-in zoom-in-95 duration-300 font-black italic">
          <form onSubmit={handleSalvarCartao} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4">
              ...
              <button type="submit" className={`w-full ${theme.primary} py-5 rounded-[2rem] uppercase text-xs mt-2 active:scale-95 italic font-black`}>Salvar</button>
      ...
      {isAdminMenuOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[500] animate-in fade-in duration-300 font-black italic">
  ...
  return (
    <div className={`${color} p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl transition-transform active:scale-[0.98] border-black/20 flex flex-col justify-between h-32 md:h-36 text-white text-left font-black italic`}>
      <div className="flex justify-between items-start w-full leading-normal">
        <span className="text-white/40 font-black text-[7px] md:text-[10px] uppercase tracking-widest italic leading-normal">{title}</span>
