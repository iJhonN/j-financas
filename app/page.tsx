  ...
  return (
    <>
      ...
      {view === 'auth' ? (
        ...
        <Dashboard user={user} theme={THEMES[theme]} currentThemeName={theme} setTheme={setTheme} onLogout={() => { supabase.auth.signOut(); window.location.reload(); }} showAlert={showAlert} />
      )}
    </>
  );
}
...
function Dashboard({ user, theme, currentThemeName, setTheme, onLogout, showAlert }: any) {
  ...
  const handleSalvarGasto = async (e: React.FormEvent) => {
    ...
    fetchDados(); setIsModalOpen(false); setDescricao(''); setValorDisplay(''); showAlert("Salvo!", 'success');
  };
  ...
  return (
    <div className="min-h-screen bg-slate-955 p-2 md:p-8 text-white font-sans overflow-x-hidden pb-20 italic font-black">
        ...
        <div className="flex justify-between items-center w-full">
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              <h1 className="text-lg md:text-xl font-black uppercase tracking-tighter px-1 leading-normal">J FINANÇAS</h1>
              <p className={`text-[9px] md:text-[10px] font-black ${theme.text} mt-1 uppercase`}>Olá, {user?.user_metadata?.full_name?.split(' ')[0]}</p>
            </div>
          </div>
          <div className="flex gap-2">
            {isAdmin && <button onClick={async () => { const { count } = await supabase.from('profiles').select('*', { count: 'exact', head: true }); setTotalUsuarios(count || 0); setIsAdminMenuOpen(true); }} className="bg-amber-500 text-slate-950 p-2.5 rounded-full active:scale-95 shadow-lg shadow-amber-500/20"><ShieldCheck size={20} /></button>}
            <div className="relative">
              <button onClick={() => setIsProfileMenuOpen(!isProfileMenuOpen)} className="bg-slate-800 text-slate-300 p-2.5 rounded-full border border-slate-700 hover:bg-blue-600 transition-all relative"><UserCircle size={20} />{notifPermission !== 'granted' && <span className="absolute top-0 right-0 w-2 h-2 bg-rose-500 rounded-full animate-pulse border border-slate-900"></span>}</button>
              {isProfileMenuOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-slate-900 border-2 border-slate-800 rounded-[2rem] shadow-2xl z-[110] overflow-hidden animate-in fade-in slide-in-from-top-2">
                  <div className="p-2 space-y-1">
                    <button onClick={() => { setIsProfileMenuOpen(false); setIsConfigModalOpen(true); }} className="w-full flex items-center gap-3 p-4 hover:bg-slate-800 rounded-2xl transition-all"><Settings className={theme.text} size={18} /><span className="text-[10px] uppercase">Ajustes / Tema</span></button>
                    <button onClick={onLogout} className="w-full flex items-center gap-3 p-4 hover:bg-rose-900/20 text-rose-500 rounded-2xl transition-all border-t border-slate-800/50 mt-1"><LogOut size={18} /><span className="text-[10px] uppercase">Sair</span></button>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6">
        ...
        <div className="relative">
          <button onClick={() => setIsFilterMenuOpen(!isFilterMenuOpen)} className="w-full bg-slate-900 p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl border-b-8 border-amber-500 flex flex-col justify-between h-32 md:h-36 text-left active:scale-95">
            ...
            <div className="flex items-center justify-between w-full"><div className="text-sm md:text-2xl font-black truncate uppercase italic px-1 leading-tight">{filtroCartao}</div><ChevronDown size={16} className={`text-amber-500 transition-transform ${isFilterMenuOpen ? 'rotate-180' : ''}`} /></div>
          </button>
          {isFilterMenuOpen && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900 border-2 border-slate-800 rounded-3xl shadow-2xl z-[500] overflow-hidden animate-in fade-in slide-in-from-top-2">
              <div className="p-2 max-h-64 overflow-y-auto font-black text-[10px] uppercase">
                ...
                {cartoes.map(c => <button key={c.id} onClick={() => { setFiltroCartao(c.banco); setIsFilterMenuOpen(false); }} className={`w-full text-left p-4 hover:bg-slate-800 rounded-2xl border-t border-slate-800/50 ${theme.text}`}>{c.banco} <br/><span className="text-[7px] text-slate-500 font-bold lowercase">{c.nome_cartao}</span></button>)}
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-slate-900 p-5 md:p-8 rounded-[2rem] border border-slate-800 h-full overflow-hidden flex flex-col shadow-2xl min-h-[500px]">
          <h2 className="text-white font-black mb-4 uppercase text-[10px] tracking-widest italic leading-normal">Lançamentos</h2>
          <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar font-black italic flex-1">
            {transacoesFiltradas.length > 0 ? transacoesFiltradas.map((t) => (
              <div key={t.id} className="flex justify-between items-center p-4 bg-slate-800/40 rounded-2xl border border-slate-800 hover:bg-slate-800/60 transition-all leading-normal">
                <div className="flex-1 min-w-0 mr-3"><p className="font-black text-slate-200 text-[10px] uppercase truncate mb-1">{t.descricao}</p><p className="text-[8px] text-slate-500 font-bold uppercase">{t.data_ordenacao}</p></div>
      ...
      {isConfigModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[2000] animate-in fade-in zoom-in-95 font-black italic">
          <form onSubmit={handleUpdateProfile} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-10 border-4 border-slate-800 shadow-2xl text-white">
            <div className="flex justify-between items-center mb-8 px-1"><h2 className="text-xl uppercase tracking-widest">Ajustes</h2><button type="button" onClick={() => setIsConfigModalOpen(false)} className="bg-slate-800 p-2 rounded-full text-slate-500 active:scale-95"><X size={20} /></button></div>
            <div className="mb-8"><p className="text-[8px] text-slate-500 uppercase mb-4 flex items-center gap-2"><Palette size={12}/> Estilo do App</p><div className="flex justify-between px-2">{Object.keys(THEMES).map((tName) => <button key={tName} type="button" onClick={() => changeTheme(tName as any)} className={`w-10 h-10 rounded-full border-4 ${currentThemeName === tName ? 'border-white scale-110 shadow-2xl' : 'border-transparent opacity-40'} ${THEMES[tName as keyof typeof THEMES].primary} transition-all`} />)}</div></div>
            <div className="space-y-4 font-black"><input value={novoNome} onChange={(e) => setNovoNome(e.target.value.toUpperCase())} placeholder="NOME" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-white uppercase text-[10px] font-black" /><input type="password" value={novaSenha} onChange={(e) => setNovaSenha(e.target.value)} placeholder="NOVA SENHA" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white text-[10px]" /><button type="submit" className={`w-full ${theme.primary} py-5 rounded-[2rem] uppercase text-xs mt-2 font-black italic shadow-lg ${theme.shadow}`}>Salvar Tudo</button></div>
          </form>
        </div>
      )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1000] animate-in fade-in zoom-in-95 duration-300 font-black italic">
          <form onSubmit={handleSalvarGasto} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-10 border-4 border-slate-800 shadow-2xl text-white">
            <div className="flex justify-between items-center mb-8 px-1"><h2 className="text-xl uppercase tracking-widest">Novo Gasto</h2><button type="button" onClick={() => setIsModalOpen(false)} className="bg-slate-800 p-2 rounded-full text-slate-500 active:scale-95"><X size={20} /></button></div>
            <div className="space-y-4"><input value={descricao} onChange={(e) => setDescricao(e.target.value.toUpperCase())} placeholder="DESCRIÇÃO" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none uppercase text-[10px] font-black" required /><div className="relative"><span className={`absolute left-4 top-1/2 -translate-y-1/2 ${theme.text} text-sm px-1`}>R$</span><input type="text" value={valorDisplay} onChange={(e) => setValorDisplay(aplicarMascara(e.target.value))} placeholder="0,00" className={`w-full pl-10 p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 ${theme.text} text-lg outline-none font-black`} required /></div>
              <select value={formaPagamento} onChange={(e) => setFormaPagamento(e.target.value)} className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 text-white uppercase text-[10px] outline-none"><option value="Pix">Pix / Dinheiro</option>{cartoes.map(c => <option key={c.id} value={`${c.banco} - ${c.nome_cartao}`}>{c.banco} ({c.nome_cartao})</option>)}</select>
              <div className="flex justify-center"><input type="date" value={data} onChange={(e) => setData(e.target.value)} className="w-auto p-3 bg-slate-800 rounded-xl border-2 border-slate-700 text-white text-[10px] outline-none text-center font-black" required /></div>
              <button type="submit" className={`w-full ${theme.primary} text-white py-5 rounded-[2rem] shadow-xl hover:bg-blue-700 active:scale-95 transition-all uppercase text-xs mt-2 italic font-black`}>Lançar Agora</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1000] animate-in fade-in duration-300 font-black italic">
          <form onSubmit={handleSalvarCartao} className="bg-slate-900 w-full max-w-sm rounded-[3rem] p-6 md:p-10 border-4 border-slate-800 shadow-2xl text-white">
            ...
            <div className="space-y-4 font-black">
              ...
              <div><input value={nomeCartao} onChange={(e) => handleNomeCartaoChange(e.target.value)} placeholder="APELIDO DO CARTÃO" className="w-full p-4 bg-slate-800 rounded-2xl border-2 border-slate-700 outline-none text-white uppercase text-[10px] font-black" required /><p className="text-[7px] text-slate-500 ml-2 mt-1 uppercase">Apenas letras. Máximo 12 caracteres.</p></div>
      ...
      {isAdminMenuOpen && (
        <div className="fixed inset-0 bg-white/10 backdrop-blur-md flex items-center justify-center p-4 z-[1500] animate-in fade-in duration-300 font-black italic">
          <div className="bg-slate-900 w-full max-w-sm rounded-[3rem] border-4 border-amber-500/30 shadow-2xl overflow-hidden font-black">
            <div className="bg-amber-500 p-6 flex justify-between items-center text-slate-950 font-black"><div><h2 className="uppercase tracking-tighter text-xl px-1">Admin Panel</h2><p className="text-[10px] uppercase mt-1">Status Global</p></div><button onClick={() => setIsAdminMenuOpen(false)} className="bg-slate-950/20 p-2 rounded-full"><X size={24} /></button></div>
            <div className="p-8 flex flex-col items-center justify-center bg-slate-800/50 m-4 rounded-[2.5rem] border-2 border-amber-500/10">
                 <Users size={48} className="text-amber-500 mb-2 opacity-50" /><span className="text-[10px] text-slate-500 uppercase tracking-widest leading-normal">Total Usuários</span>
...
function Card({ title, value, icon, color }: any) {
  return (
    <div className={`${color} p-4 md:p-7 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl transition-transform active:scale-[0.98] border-black/20 flex flex-col justify-between h-32 md:h-36 text-white text-left font-black italic`}>
      <div className="flex justify-between items-start w-full leading-normal"><span className="text-white/40 font-black text-[7px] md:text-[10px] uppercase tracking-widest italic">{title}</span><div className="p-1.5 md:p-3 bg-white/5 rounded-xl backdrop-blur-md border border-white/5 opacity-50">{icon}</div></div>
