...
export default function LoginPage() {
  ...
  return (
    <>
      ...
      {/* h-screen e overflow-hidden garantem que não haja scroll */}
      <div className="h-screen w-full flex flex-col items-center justify-center p-4 text-white font-black italic relative z-10 overflow-hidden">
        
        <form 
          onSubmit={handleAuth} 
          className="bg-[#111827]/80 backdrop-blur-xl w-full max-w-md rounded-[2.5rem] p-6 md:p-10 border-4 border-slate-800/50 shadow-2xl transition-all"
        >
          {/* Header - Tamanho da logo ajustado para mobile (w-24 -> w-32) */}
          <div className="flex flex-col items-center mb-6 md:mb-10 text-center">
            <div className="relative mb-4">
              ...
              <img 
                ...
                className="relative w-24 h-24 md:w-40 md:h-40 object-contain drop-shadow-[0_0_15px_rgba(37,99,235,0.2)]" 
              />
            </div>
            <h1 className="text-lg md:text-2xl font-black uppercase italic tracking-tighter">
          ...
          <div className="space-y-4 md:space-y-5">
                ...
                <div className="relative group">
                  {authMode === 'signup' && !password && (
                    <div className="flex items-center gap-1 mb-1 text-[7px] md:text-[8px] text-blue-400 font-black uppercase tracking-[0.2em] animate-pulse">
                      <Info size={10} />
                      Use 10+ chars, 1 Maiúscula e 1 Símbolo
                    </div>
                  )}
            ...
            {step === 1 && (
              <button type="button" onClick={() => setAuthMode(authMode === 'login' ? 'signup' : 'login')} className="w-full text-center text-[9px] text-slate-500 mt-4 cursor-pointer uppercase hover:text-blue-400 transition-all font-black">
                {authMode === 'login' ? 'Criar conta' : 'Já sou membro'}
              </button>
            )}
          </div>
        </form>

        {/* Rodapé fixo - Ajustado para bottom-4 no mobile */}
        <div className="mt-6 flex flex-col items-center opacity-30 hover:opacity-100 transition-all duration-700">
